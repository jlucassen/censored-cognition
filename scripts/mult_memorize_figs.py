...
for name in judges:
    judge_results_3_flat = JudgeResult.from_json(f'logs/multiplication/mult_memorize_judge_results_3_{name}.jsonl')
    judge_results_4_flat = JudgeResult.from_json(f'logs/multiplication/mult_memorize_judge_results_4_{name}.jsonl')
    ...
    make_plot(judge_results_3, judge_results_4, title_addendum=name, savefilename=f'mult_memorize_{name}')